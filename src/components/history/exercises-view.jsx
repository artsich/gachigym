...
const getExerciseInformation = (exercises, activeExercise) => {
	const tonnage = exercises.length
		? exercises[activeExercise].sets?.reduce((tonnage, set) => {
				return tonnage + (set.weight || 0) * (set.reps || 1);
		  }, 0)
		: 0;

	const reps = exercises.length
		? exercises[activeExercise].sets?.reduce((summaryReps, set) => {
